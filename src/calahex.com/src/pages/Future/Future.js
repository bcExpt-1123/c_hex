...
const CryptoExchange = (props) => {
    const { selectLanding, isAuthenticated, cryptoLists, cryptoOrderLists, cryptoTradLists } = props;
    ...
    useEffect(() => {
        ...
        if (!Object.keys(selectRow).length && cryptoLists.length) {
            setSelectRow(cryptoLists.filter(crypto => crypto.name == 'USDT_BTC')[0]);
        }
    }, [cryptoLists])
            ...
            <TabPanel value={selNum} index={tabNum}>
                ...
                <div className="orderSellBook">
                    <TableContainer className="order-buy-table-container">
                        <Table className={classes.table} size="small" aria-label="a dense table">
                            <TableBody>
                                {cryptoOrderLists.asks.reverse().map((row, i) => (
                                    <Tooltip key={i} type={1} title={
                                        ...
                                        <BgTableRow total={row.total} max={cryptoOrderLists.asks[cryptoOrderLists.asks.length - 1].sum_total}>
    ...
    if (isAuthenticated) {
        adminPan = <Grid item className="adminPanLogin">
            <AppBar position="static" color="transparent">
                ...
                >
                    <StyledTab className="tabButton" label="Buy"  {...a11yProps(0)} />
                    <StyledTab className="tabButton" label="Sell"  {...a11yProps(1)} />
                </StyledTabs>
            </AppBar>
            <TabPanel value={adminPanLogin} index={0}>
                ...
                <Grid container spacing={3}>
                    <Grid item xs={3}><Button size="small" type="primary" className="color-primary" >25%</Button></Grid>
                    <Grid item xs={3}><Button size="small" type="primary" className="color-primary" >50%</Button></Grid>
                    <Grid item xs={3}><Button size="small" type="primary" className="color-primary" >75%</Button></Grid>
                    <Grid item xs={3}><Button size="small" type="primary" className="color-primary" >100%</Button></Grid>
                </Grid>
                <div><span>fee: 0.125%/0.125%</span></div>
                <Button fullWidth size="small" className="color-primary">Buy</Button>

                <div style={{ marginTop: 50 }}>
                    <div className="unitsize"><div><p>ETH</p><p>Etherum</p></div><div><p>0.000000</p>
                        <p>$0.00</p></div></div>
                    <div className="unitsize"><div><p>ETH</p><p>Etherum</p></div><div><p>0.000000</p>
                        <p>$0.00</p></div></div>
                </div>
                <Grid container spacing={3}>
                    <Grid item xs={6}><Button size="small" type="primary" className="color-primary" fullWidth>Deposit</Button></Grid>
                    <Grid item xs={6}><Button size="small" type="primary" className="color-primary" fullWidth>Transfer</Button></Grid>
                </Grid>
            </TabPanel>
    ...
    return (
        <FutureDiv className={classes.root} show={show} responsive={hiddenC} match={!match2} match1={!match1} >
                ...
                <Grid item className="sider1">
                    <div className={classes.xrpSearch}>
                        <div className={classes.xrpfilter}>
                            ...
                            <span className={classes.xrpSpan} onClick={() => { selectCrypto('BTC'); }}>BTC</span>
                                ...
                                <TableBody>
                                    {cryptoOrderLists.asks.reverse().map((row, i) => (
                                        <Tooltip key={i} type={1} title={
                                            ...
                                            <BgTableRow total={row.sum_total} max={cryptoOrderLists.asks[cryptoOrderLists.asks.length - 1].sum_total}>
                                                <TableCell align="right" className="minus">{Number(row.price).toFixed(size)}</TableCell>
                                                <TableCell align="right">{row.amount}</TableCell>
                                                <TableCell align="right">{row.total}</TableCell>
                                            </BgTableRow>
                        ...
                        <Grid item className="tradingViews">
                            <AppBar position="static" color="transparent">
                                ...
                                >
                                    ...
                                    {!match1 && tabHeader('Market Trade', 2)}
                                    {!match2 && tabHeader('Order Book', 3)}
                            ...
                            <TabPanel value={tradingViewTab} index={1}>
                                <DepthChart data={cryptoOrderLists} />
                            </TabPanel>
                            {!match1 && tabbodyMarketTrade(2, tradingViewTab)}
                            {!match2 && tabbodyOrderBook(3, tradingViewTab)}
